...
export async function GET(req:any){
    ...
    try {
        ...
        const [periodo]:any = await conexion.query(`SELECT per_id,grado_base,grupo_id FROM periodo_academicos
        ...
        const [save]:any = await conexion.query(`INSERT INTO newObservacionesProcesos(grado,grupo,periodo,escala,relacionBanco,cga) VALUES('${periodo[0]?.grado_base}','${periodo[0]?.grupo_id}','${periodo[0]?.per_id}','${escala}','${id}','${cga}')`)

        return NextResponse.json({body: save},{status:200})
    } catch (error) {
        console.log(error)
        return NextResponse.json({body: "Error al enviar la información"},{status: 400})
    }
}