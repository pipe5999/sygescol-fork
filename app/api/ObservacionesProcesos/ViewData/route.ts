...
try {
    const [data]: any = await conexion.query(`SELECT * FROM newBancoObservacionesProcesos BP INNER JOIN newObservacionesProcesos PE ON PE.relacionBanco = BP.id WHERE PE.id = ${id}`);
