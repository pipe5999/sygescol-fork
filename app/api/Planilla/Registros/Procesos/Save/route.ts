...
export async function GET(req: any) {
  ...
  try {
    ...
    const [search]: any = await conexion.query(
      `SELECT * FROM newProcesoEstudiante NPE INNER JOIN newProcesosEvaluacion PE ON NPE.proceso = PE.id WHERE estudiante = ${estudiante} AND NPE.cga = ${procesoFind[0]?.cga}`
    );
    search?.map((pros: any) => {
    ...
    if (correcto == 0) {
      const [save]: any = await conexion.query(
        `INSERT INTO newProcesoEstudiante(estudiante,proceso,periodo,cga) VALUES('${estudiante}','${proceso}','${procesoFind[0]?.periodo}','${procesoFind[0]?.cga}')`
      );
      ...
      return NextResponse.json(
        {
          body: "El estudiante ya tiene asignado procesos para otro desempeño",
        },
        { status: 200 }
      );
    }
